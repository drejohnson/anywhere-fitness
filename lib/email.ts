...
export const sendPasswordResetEmail = (email: string, resetToken: string) => {
  ...
  const resetUrl = `${process.env.FRONTEND_DOMAIN}/accounts/reset?token=${resetToken}`

  const content = {
    from: process.env.MAILTRAP_EMAIL as string,
    to: email,
...
export const sendAccountVerificationEmail = (email: string, verifyToken: string) => {
  // We'll redirect to the backend here
  const confirmUrl = `${process.env.BACKEND_DOMAIN}/api/accounts/confirm/${verifyToken}`
...
const sendMailTrapEmail = (mailContent: MailContent) => {
